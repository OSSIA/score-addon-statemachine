...
ProcessExecutorComponent::ProcessExecutorComponent(
    statemachine::Model& element, const Execution::Context& ctx,
    const Id<score::Component>& id, QObject* parent)
    : ProcessComponent_T{element, ctx, id, "statemachineExecutorComponent", parent}
{
  auto n = std::make_shared<statemachine::node>();
  this->node = n;
   ...
   *         this, [=] (const QColor& c) {
   *
   *   in_exec([c,n=std::dynamic_pointer_cast<statemachine::node>(this->node)] {
   *     n->set_color(c);
   *   });
   *
   * });
